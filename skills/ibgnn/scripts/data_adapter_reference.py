"""IBGNN data adapter reference — reuses BrainGNN's PyG format.

Key design decisions:
1. IBGNN's IBGConv takes the same PyG Data graph as BrainGNN (x=[N,N] FC rows,
   edge_index, edge_attr).
2. No new data adapter needed — `models/ibgnn/scripts/data_adapter.py` is a
   thin re-export of `models.braingnn.scripts.data_adapter`.
3. IBGNN internally calls `edge_attr.abs()` before GCN normalization, so the
   data layer can keep signed Pearson r weights.

Data path convention:
    data/braingnn_input/<atlas>/sub-<id>.pt   (fMRI connectivity, shared with BrainGNN)

Full implementation: models/ibgnn/scripts/data_adapter.py
"""
...
def load_single_subject_as_pyg(atlas: str, subject_id: str) -> Data:
    """Minimal example: load one subject as a PyG Data object for IBGNN.
