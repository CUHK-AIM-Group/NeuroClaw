...
2. If nvidia-smi fails → immediately print the exact NVIDIA Container Toolkit installation commands below and instruct user to run them manually before retry.

3. If paths not provided → interactively ask user for FLAIR and T1w full paths and confirm they exist on disk.
...
6. Generate numbered execution plan.

7. Present plan, estimated runtime (~5–15 min on GPU), requirements and risks → wait for explicit user confirmation (“YES” / “execute” / “proceed”).
...
**Key design principle (2026 update)**: All Docker execution routed through `claw-shell`.
...
Installation of Docker delegated to `dependency-planner`.  
...
- `dependency-planner`   → install Docker
