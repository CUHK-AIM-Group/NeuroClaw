"""Incremental ingest: add INDIVIDUAL_DATA anchors + bridges to an existing KG.

Usage:
    python -m neurooracle.src.apply_individual_data \
...
def main() -> None:
    ...
    log = logging.getLogger("apply_individual_data")
    ...
    log.info("=== Ingesting INDIVIDUAL_DATA Anchors ===")
    a_counts = ingest_individual_data_anchors(kg)
    log.info(f"  anchors: {a_counts}")

    log.info("=== Ingesting INDIVIDUAL_DATA Bridges ===")
    b_counts = ingest_individual_data_bridges(kg)
    log.info(f"  bridges: {b_counts}")

    post = kg.stats()
    log.info(f"  after:  {post['n_concepts']} concepts / {post['n_edges']} edges "
             f"(+{post['n_concepts']-pre['n_concepts']} concepts, "
