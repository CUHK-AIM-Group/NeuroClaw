...
const PER_QUERY = Number(process.env.GENMED_PER_QUERY || 1000);
const EMAIL = process.env.NCBI_EMAIL || 'neuroclaw@example.com';
const API_KEY = process.env.NCBI_API_KEY || '';
const RUN_LABEL = process.env.GENMED_RUN_LABEL || `supplement_${YEAR_START}_${YEAR_END}_broad`;
const CURATION_ROUND = process.env.GENMED_CURATION_ROUND || 'general_neuromed_1980_present_broad_supplement';
...
function parseArticles(xml) {
  ...
  for (const block of blocks) {
    ...
    if (pmid && title && abstract) out.push({ pmid, doi, title, authors, year, journal, abstract });
  }
  return out;
}
...
function buildQuery(disease, year) {
  ...
  return `(${disease}[Title/Abstract]) AND (${neuroscience}) AND hasabstract[text] AND ${year}:${year}[pdat] NOT (${weakTitle})`;
}

function isAcceptable(ref) {
  const title = (ref.title || '').toLowerCase();
  if (WEAK_TOPIC_TITLE_TERMS.some((term) => title.includes(term))) return false;
  if (!ref.abstract || ref.abstract.length < 80) return false;
  return true;
}
...
async function fetchJson(url, params, retries = 5) {
  ...
  if (!cleanParams.api_key) delete cleanParams.api_key;
  for (const [k, v] of Object.entries(cleanParams)) u.searchParams.set(k, String(v));
...
async function fetchTextPost(url, body, retries = 5) {
  ...
  if (!cleanBody.api_key) delete cleanBody.api_key;
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
...
async function main() {
  ...
  console.log(`starting broad supplement: queue=${queue.length}, unique_seen=${seenPmids.size}, target_new=${TARGET_NEW}`);
  for (const { disease, year } of plan) {
    ...
    for (const ref of articles) {
      ...
      if (!isAcceptable(ref)) continue;
      rows.push({
        batch_index: nextIndex++,
        queue_id: `GENMEDBROAD:${ref.pmid}`,
    ...
    if (rows.length || freshPmids.length) {
      console.log(`${year} ${disease}: +${rows.length} queued; fresh=${freshPmids.length}; fetched=${articles.length}; total_added=${added}/${TARGET_NEW}`);
    }
    await sleep(260);
  }
  ...
  const summary = {
    created_at: new Date().toISOString(),
    objective: 'broader PubMed supplement for general manual queue, 1980-present',
    stage,
