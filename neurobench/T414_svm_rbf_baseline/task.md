# T414_svm_rbf_baseline: SVM-RBF Baseline
## Task Description

Train and evaluate the SVM-RBF Baseline on vectorized FC features for the standard settings. SVM with RBF kernel; C/gamma via inner CV.
...
- Use `models/train_unified.py --model svm_rbf` for both settings.
...
- Save artefacts under `models/benchmark_results/T414_svm_rbf_baseline/<setting>/`.

- Save checkpoints under `models/checkpoints/svm_rbf/<atlas>/fold{k}.pt`.
