# T402_cnn2d_fc_train_eval: 2D-CNN on FC Matrix Training and Evaluation
## Task Description

Train and evaluate 2D-CNN on FC Matrix on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. 2D CNN treating the FC matrix as an image (symmetric input; document upper-triangle handling).
...
- Use `models/train_unified.py --model cnn2d_fc` for both settings.
...
- Save artefacts under `models/benchmark_results/T402_cnn2d_fc_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/cnn2d_fc/<atlas>/fold{k}.pt`.
