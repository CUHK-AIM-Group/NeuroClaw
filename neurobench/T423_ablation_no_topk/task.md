# T423_ablation_no_topk: Ablation: BrainGNN without TopK Pooling
## Task Description

Ablate BrainGNN's TopK pooling (replace with global mean pooling); quantify the contribution of pooling to both settings.

## Input Requirement

Required input(s):

- ROI-level FC matrices per subject (NPZ), built from a chosen atlas (e.g. `schaefer_200_7net`, `aal_116`)

- Subject list file (`ready_subjects.txt`)

- Labels CSV (HCP age: `data/hcp_age_labels.csv`; ABIDE dx: `data/abide_dx_labels.csv`)

- Atlas name and ROI count (must match FC dimension)


If any required input is missing, return:

- Missing required input


## Constraints

- Single-change principle: only the pooling differs.

- Save artefacts under `models/benchmark_results/T423_ablation_no_topk/`.


## Expected Output

Expected output artifact(s):

- Ablation metrics CSV vs. full model

- `ablation_report.md`

- `result_YYYYMMDD_HHMMSS.json`


Recommended metadata file:

- result_YYYYMMDD_HHMMSS.json


## Evaluation

- This test case is manually evaluated.
