# T408_dann_site_train_eval: Domain-Adversarial Site Adaptation Training and Evaluation
## Task Description

Train and evaluate Domain-Adversarial Site Adaptation on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. DANN with site-discriminator gradient reversal on top of a GCN backbone, targeting site-invariant ABIDE features.
...
- Use `models/train_unified.py --model dann_site` for both settings.
...
- Save artefacts under `models/benchmark_results/T408_dann_site_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/dann_site/<atlas>/fold{k}.pt`.
