# T403_gru_ts_train_eval: GRU on ROI Time Series Training and Evaluation
## Task Description

Train and evaluate GRU on ROI Time Series on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. 2-layer GRU over ROI time series, last-hidden readout.
...
- Use `models/train_unified.py --model gru_ts` for both settings.
...
- Save artefacts under `models/benchmark_results/T403_gru_ts_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/gru_ts/<atlas>/fold{k}.pt`.
