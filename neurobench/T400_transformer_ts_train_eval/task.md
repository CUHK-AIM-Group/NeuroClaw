# T400_transformer_ts_train_eval: ROI Time-Series Transformer Training and Evaluation
## Task Description

Train and evaluate ROI Time-Series Transformer on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. Vanilla transformer over ROI time series (CLS token readout); positional encoding documented.
...
- Use `models/train_unified.py --model transformer_ts` for both settings.
...
- Save artefacts under `models/benchmark_results/T400_transformer_ts_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/transformer_ts/<atlas>/fold{k}.pt`.
