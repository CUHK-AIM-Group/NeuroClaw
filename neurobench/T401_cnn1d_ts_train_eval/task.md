# T401_cnn1d_ts_train_eval: 1D-CNN on ROI Time Series Training and Evaluation
## Task Description

Train and evaluate 1D-CNN on ROI Time Series on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. 1D CNN over ROI time series with global average pooling readout.
...
- Use `models/train_unified.py --model cnn1d_ts` for both settings.
...
- Save artefacts under `models/benchmark_results/T401_cnn1d_ts_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/cnn1d_ts/<atlas>/fold{k}.pt`.
