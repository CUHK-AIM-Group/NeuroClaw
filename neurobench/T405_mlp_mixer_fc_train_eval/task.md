# T405_mlp_mixer_fc_train_eval: MLP-Mixer on FC Training and Evaluation
## Task Description

Train and evaluate MLP-Mixer on FC on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. MLP-Mixer over vectorized FC (ROI-mixing + feature-mixing blocks).
...
- Use `models/train_unified.py --model mlp_mixer_fc` for both settings.
...
- Save artefacts under `models/benchmark_results/T405_mlp_mixer_fc_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/mlp_mixer_fc/<atlas>/fold{k}.pt`.
