# T397_gin_train_eval: GIN Training and Evaluation
## Task Description

Train and evaluate GIN on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. Graph Isomorphism Network (2 layers, MLP epsilon-learnable) on thresholded FC graphs.
...
- Use `models/train_unified.py --model gin` for both settings.
...
- Save artefacts under `models/benchmark_results/T397_gin_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/gin/<atlas>/fold{k}.pt`.
