# T396_graphsage_train_eval: GraphSAGE Training and Evaluation
## Task Description

Train and evaluate GraphSAGE on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. GraphSAGE (mean aggregator, 2 layers) on ROI graphs; node features from FC rows.
...
- Use `models/train_unified.py --model graphsage` for both settings.
...
- Save artefacts under `models/benchmark_results/T396_graphsage_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/graphsage/<atlas>/fold{k}.pt`.
