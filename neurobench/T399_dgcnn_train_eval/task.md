# T399_dgcnn_train_eval: DGCNN (EdgeConv) Training and Evaluation
## Task Description

Train and evaluate DGCNN (EdgeConv) on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. DGCNN with EdgeConv layers on k-NN graphs built from FC (k=10); document graph construction.
...
- Use `models/train_unified.py --model dgcnn` for both settings.
...
- Save artefacts under `models/benchmark_results/T399_dgcnn_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/dgcnn/<atlas>/fold{k}.pt`.
