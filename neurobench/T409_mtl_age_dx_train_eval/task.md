# T409_mtl_age_dx_train_eval: Multi-Task Learning (age + dx) Training and Evaluation
## Task Description

Train and evaluate Multi-Task Learning (age + dx) on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. Joint multi-task model: shared GNN trunk with age-regression and dx-classification heads; loss weighting documented.
...
- Use `models/train_unified.py --model mtl_age_dx` for both settings.
...
- Save artefacts under `models/benchmark_results/T409_mtl_age_dx_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/mtl_age_dx/<atlas>/fold{k}.pt`.
