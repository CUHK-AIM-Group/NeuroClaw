# T406_autoencoder_pretrain_train_eval: Autoencoder Pretraining + Probe Training and Evaluation
## Task Description

Train and evaluate Autoencoder Pretraining + Probe on preprocessed functional connectivity data for two settings: HCP age regression and ABIDE diagnosis classification. Pretrain an FC autoencoder (reconstruction), then train a linear/ridge probe on the bottleneck for both settings.
...
- Use `models/train_unified.py --model ae_pretrain` for both settings.
...
- Save artefacts under `models/benchmark_results/T406_autoencoder_pretrain_train_eval/<setting>/`.

- Save checkpoints under `models/checkpoints/ae_pretrain/<atlas>/fold{k}.pt`.
