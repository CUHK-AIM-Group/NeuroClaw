# T412_xgboost_baseline: XGBoost Baseline
## Task Description

Train and evaluate the XGBoost Baseline on vectorized FC features for the standard settings. Gradient-boosted trees on vectorized FC features (upper triangle).
...
- Use `models/train_unified.py --model xgboost` for both settings.
...
- Save artefacts under `models/benchmark_results/T412_xgboost_baseline/<setting>/`.

- Save checkpoints under `models/checkpoints/xgboost/<atlas>/fold{k}.pt`.
