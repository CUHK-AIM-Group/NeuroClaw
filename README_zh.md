...
- **[2026.05.06]**：新增 14 个数据集技能（`camcan-skill`、`ixi-skill`、`mschallenge-skill`、`mnd-skill`、`nsd-skill`、`nifd-skill`、`oasis-skill`、`pnc-skill`、`ppmi-skill`、`rest-mneta-mdd-skill`、`seed-iv-skill`、`seed-vig-skill`、`tcp-skill`、`ucla-cnp-skill`）及配套脚本；将 `hcp-skill` 拆分为 4 个数据集专用技能（`hcpya-skill`、`hcpa-skill`、`hcpd-skill`、`hcpep-skill`）；新增 3 个模态技能（`pet-skill`、`asl-skill`、`meg-skill`）；全部 81 个技能统一元数据格式（`layer`、`skill_type`、`dependencies`）；skill-loader DAG 验证确保依赖图无环。
...
| Longitudinal MS Lesion Segmentation Challenge | T1w; T2w; FLAIR; PD | 专家手动病灶分割标注，用于 MS 分割基准 | 5 名 MS 患者的多时间点纵向数据 | https://smart-stats-tools.org/lesion-challenge |
