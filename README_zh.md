...
### 🔐 Harness 工程
- **自我验证**：所有技能输出的内置验证检查（BIDS 合规性、数据完整性、统计边界、工件质量）
- **可复现日志**：时间戳 JSONL 审计日志，SHA256 哈希与 PII 清理，确保完整实验可追踪
- **检查点与恢复**：自动阶段检查点（LZ4 压缩）；从最后验证状态恢复中断任务
- **漂移检测**：连续监控数据/模型分布变化（KL 散度、延迟、失败率），支持可配置阈值
- **环境可复现性**：自动捕获与验证 Python 版本、包依赖、CUDA 与系统配置
